...
public class ConfigUtils {
    ...
    private static List<Config> configs = new CopyOnWriteArrayList<>();

    public static String v(String key) throws Exception{
        ...
        throw new Exception("config "+key+" noFound");
    }
     ...
     * @throws Exception
     */
    public static String v(String key,Boolean ignoreNotFound) throws Exception {
        if( null == key || key.trim().isEmpty() )
            throw new IllegalArgumentException("config key must is not null");
        for(Config config : configs){
            String v = config.v(key);
